  ...
  const renderContent = () => {
    ...
    return (
      <>
        ...
        <SectionContainer id="portfolio" padding="80px 20px">
          <div style={{ textAlign: 'center', marginBottom: '40px' }}>
            <h2>공간별 시공 사례</h2>
            <p style={{ opacity: 0.8, marginTop: '8px', maxWidth: '700px', marginLeft: 'auto', marginRight: 'auto', lineHeight: 1.6, fontSize: '0.95rem' }}>
              탄성코트와 줄눈시공은 공간과 기존 상태에 따라 작업 범위가 달라집니다. 실제 시공 사례에서 작업 전 상태와 마감 범위를 확인해 보세요.
            </p>
          </div>

          {activeGroup === 'both' && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginBottom: '32px' }}>
              {['전체', '탄성코트', '줄눈시공'].map(type => (
                <button
                  key={type}
                  ...
                  style={{
                    ...
                    backgroundColor: portfolioFilter === type ? 'var(--forest-green-main)' : 'transparent',
                    color: portfolioFilter === type ? 'var(--white)' : 'var(--forest-green-main)',
                    transition: 'all 0.2s',
                    fontWeight: '500'
                  }}
                >
          ...
          <div style={{
            ...
            {filteredPortfolios[0] && (
              <div
                ...
                onClick={() => setSelectedPortfolio(filteredPortfolios[0])}
              >
                <ImagePlaceholder label={filteredPortfolios[0].detailImage} ratio="16:9" size="Recommended: 800x450" />
                <span style={{ display: 'inline-block', marginTop: '16px', fontSize: '0.8rem', color: 'var(--forest-green-sub)', textTransform: 'uppercase', fontWeight: 'bold' }}>
                  {filteredPortfolios[0].serviceType} · {filteredPortfolios[0].spaceType}
                </span>
                <h3 style={{ margin: '8px 0', fontSize: '1.3rem' }}>{filteredPortfolios[0].title}</h3>
                <p style={{ opacity: 0.8, fontSize: '0.9rem' }}>{filteredPortfolios[0].summary}</p>
              </div>
            )}

            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {filteredPortfolios.slice(1).map(item => (
                <div
                  key={item.id}
                  style={{
                    backgroundColor: 'var(--white)',
                ...
                >
                  <ImagePlaceholder label={item.detailImage} ratio="1:1" size="Thumbnail 200x200" />
                  <div>
                    <span style={{ fontSize: '0.75rem', color: 'var(--forest-green-sub)', fontWeight: 'bold' }}>
                      {item.serviceType}
                    </span>
                    <h4 style={{ margin: '4px 0', fontSize: '0.95rem' }}>{item.title}</h4>
                    <p style={{ opacity: 0.7, fontSize: '0.85rem' }}>{item.spaceType}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {selectedPortfolio && (
            <div style={{
              ...
              width: '100%',
              backgroundColor: 'var(--white)',
              borderTop: '2px solid var(--forest-green-main)',
              boxShadow: '0 -4px 15px rgba(0,0,0,0.1)',
              zIndex: 101,
              padding: '24px var(--mobile-side-margin)',
              maxHeight: '80vh',
              overflowY: 'auto'
            }}>
              <div style={{ maxWidth: 'var(--desktop-max-width)', margin: '0 auto', textAlign: 'left' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  <span style={{ color: 'var(--forest-green-sub)', fontWeight: 'bold', fontSize: '0.9rem' }}>
                    {selectedPortfolio.serviceType} 상세 정보
                  </span>
                  <button
                    onClick={() => setSelectedPortfolio(null)}
                    style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--forest-green-main)' }}
                  >
                    ×
                  </button>
                </div>
                <h3>{selectedPortfolio.title}</h3>
                <p style={{ margin: '8px 0 12px 0', opacity: 0.8 }}>{selectedPortfolio.summary}</p>
                <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? '1fr 1fr' : '1fr', gap: '20px', marginBottom: '20px' }}>
                  <div>
                    <ImagePlaceholder label={selectedPortfolio.beforeImage} ratio="4:3" size="Before 이미지" />
                  </div>
                  <div>
                    <ImagePlaceholder label={selectedPortfolio.afterImage} ratio="4:3" size="After 이미지" />
                  </div>
                </div>
                <PrimaryButton onClick={() => { setSelectedPortfolio(null); handleCTA(); }}>상담 신청하기</PrimaryButton>
              </div>
            </div>
          )}
        </SectionContainer>
